                ...
                for j in range(0, len(offsets)):
                    if(i != j):
                        ...
                        if(abs(separation) < self.minTimeOffset):
                            check = 0
                            """
                            if(offsets[i] + self.minTimeOffset <= deltaTmax):
                                offsets[i] += self.minTimeOffset
                            elif(offsets[i] - self.minTimeOffset >= deltaTmin):
                                offsets[i] -= self.minTimeOffset
                            elif(offsets[j] + self.minTimeOffset <= deltaTmax):
                                offsets[j] += self.minTimeOffset
                            elif(offsets[j] - self.minTimeOffset >= deltaTmin):
                                offsets[j] -= self.minTimeOffset
                            """
                        else:
                            check += 1
