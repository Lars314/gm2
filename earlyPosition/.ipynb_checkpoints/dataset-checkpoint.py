...
class DataSet:
    ...
    def set_up_calos(self, step_length, verbose=False):
        ...
        for caloNum in range(1, 25):
            ...
            nora_hist = self.file.Get("verticalPosition/clusters" + \
                                     str(caloNum)).Clone(self.name + "_calo_" + str(caloNum))
            rand_hist = self.file.Get("verticalPosition/randclusters" + \
                                      str(caloNum)).Clone(self.name + "_rand_calo_" + str(caloNum))
            ...
            this_calo = Calorimeter(number=caloNum, step=step_length, hists=this_calo_hists)
            this_calo.build_dfs()
